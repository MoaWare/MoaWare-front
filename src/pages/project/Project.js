...
import { callPorjectDeleteAPI, callPorjectProgressDoneAPI, callProjectProgressListAPI } from '../../apis/ProjectAPICalls';
...
function Project() {
    ...
    const { proDone } = useSelector(state => state.projectReducer);
    ...
    useEffect(() => {
        ...
        if(proDone?.status===200) {
            toast.success('프로젝트가 완료 되었습니다.', {
                position: toast.POSITION.TOP_CENTER, // 토스트 위치 (옵션)
                autoClose: 2000, // 자동으로 닫히는 시간 (ms) (옵션)
                hideProgressBar: false, // 진행 막대 숨김 여부 (옵션)
            });
            navigate("/project/done");
        } else if (proDone?.state === 400) {
            alert(proDone.message);
          }
      }, [delProj,proDone]);
    ...
    const onClickDone = e => {
        dispatch(callPorjectProgressDoneAPI({ projCode : selectPorjCode }))
    }
                ...
                /> }
                    ...
                    <div>
                        <button className={ProjectCSS.workBtn1}
                            onClick={ onClickDone }
                            >완료하기</button>
                        <button className={ProjectCSS.workBtn2}
